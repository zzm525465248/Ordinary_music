...
class OnSellRepository {
    ...
    suspend fun songSheet_details(id: Long)=apiService.getSongsheet_details(id).await()

    suspend fun songSheet_list(id: Long,page:Int)=apiService.getSongSheet_list(id,page,20).await()
    ...
    suspend fun User(cookie:String,id:Int)=apiService.get_User(cookie,id).await()
    ...
    //获取推荐歌单
    suspend fun get_recommended_song_list(id:String)=apiService.get_Recommended_song_list(id).await()

    //获取歌单
    suspend fun get_songsheetlist(name: String, size: Int, page:Int)=apiService.get_songsheet_list(name,size,page).await()

//    fun getOnSellList(name: String) =
//      RetrofitClient.apiService.getAppUser_Data4(name)
                ...
                override fun onResponse(call: Call<T>, response: Response<T>) {
                   val body=response.body()
                    if(body!=null){ continuation.resume(body)
                        ...
                        RuntimeException(response.body().toString())
                    )
                }
